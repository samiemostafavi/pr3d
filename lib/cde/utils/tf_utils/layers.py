...
class XavierUniformInitializer(object):
    def __call__(self, shape, dtype=tf.float64, *args, **kwargs):
        if len(shape) == 2:
...
def py_ortho_init(scale):
    def _init(shape):
        ...
        return np.cast['float64'](u * scale)
...
class OrthogonalInitializer(object):
    ...
    def __call__(self, shape, dtype=tf.float64, *args, **kwargs):
        result, = tf.py_func(py_ortho_init(self.scale), [shape], [tf.float64])
...
class NormalizationLayer(Layer):
    def __init__(self, incoming, feature_dim, name='', **kwargs):
        ...
        self.mean = self.add_param(
            tf.Variable(np.zeros(feature_dim, dtype=np.float64), name=name+'_mean', trainable=False),
            (feature_dim,),
            name=name+'mean',
            trainable=False
        )
        self.std = self.add_param(
          tf.Variable(np.ones(feature_dim, dtype=np.float64), name=name+'_std', trainable=False),
          (feature_dim,),
          name=name+'std',
          trainable=False
        )
...
class GaussianNoiseLayer(Layer):
    def __init__(self, incoming, noise_std, noise_on_ph=None, name='', **kwargs):
        ...
        self.noise_std = self.add_param(
            tf.Variable(noise_std, dtype=tf.float64, name='noise_std_' + name, trainable=False),
            (1,),
    ...
    def get_output_for(self, input, **kwargs):
        if self.noise_on_ph is not None:
            noised_input = input + tf.random_normal(tf.shape(input), stddev=self.noise_std, dtype=tf.float64)
...
class GRULayer(Layer):
    ...
    def __init__(self, incoming, num_units, hidden_nonlinearity,
        ...
        h_dummy = tf.placeholder(dtype=tf.float64, shape=(None, num_units), name="h_dummy")
        x_dummy = tf.placeholder(dtype=tf.float64, shape=(None, input_dim), name="x_dummy")
...
class TfGRULayer(Layer):
    ...
    def __init__(self, incoming, num_units, hidden_nonlinearity, horizon=None, hidden_init_trainable=False,
        ...
        input_dummy = tf.placeholder(tf.float64, (None, input_dim), "input_dummy")
        hidden_dummy = tf.placeholder(tf.float64, (None, num_units), "hidden_dummy")
...
class TfBasicLSTMLayer(Layer):
    ...
    def __init__(self, incoming, num_units, hidden_nonlinearity, horizon=None, hidden_init_trainable=False,
        ...
        input_dummy = tf.placeholder(tf.float64, (None, input_dim), "input_dummy")
        hidden_dummy = tf.placeholder(tf.float64, (None, num_units), "hidden_dummy")
        cell_dummy = tf.placeholder(tf.float64, (None, num_units), "cell_dummy")
