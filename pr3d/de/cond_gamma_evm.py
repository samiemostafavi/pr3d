...
class ConditionalGammaEVM(ConditionalDensityEstimator):
    ...
    def create_models(self):
        ...
        # build gamma density
        gamma = tfd.Gamma(
            concentration=tf.squeeze(self.gamma_shape), rate=tf.squeeze(self.gamma_rate)
        )

        # find the normalization factor (from X)
        # squeezing the tail_threshold was important
        self.norm_factor = tf.constant(1.00, dtype=self.dtype) - gamma.cdf(
    ...
    def sample_n(
        ...
        threshold_act_t = tf.convert_to_tensor(
            result_dict["tail_threshold"], dtype=self.dtype
        )
        gamma_concentration_t = tf.convert_to_tensor(
            result_dict["gamma_shape"], dtype=self.dtype
        )
        ...
        gpd_concentration_t = tf.convert_to_tensor(
